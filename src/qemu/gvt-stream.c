...
typedef struct GVTStreamDisplay {
    ...
    uint64_t publish_snapshot_count;
    uint64_t publish_snapshot_fail_count;
    int64_t last_publish_ms;
    QemuDmaBuf *last_publish_dmabuf;
    int publish_fb_index;
...
static void gvt_stream_publish_snapshot_dmabuf(GVTStreamDisplay *gdpy,
                                               QemuDmaBuf *dmabuf,
                                               int64_t now_ms,
                                               bool force)
{
#ifdef CONFIG_GBM
    g_autoptr(QemuDmaBuf) snapshot = NULL;
    egl_fb *pub_fb;
    uint32_t width, height, texture;
    int fd[DMABUF_MAX_PLANES] = { -1, -1, -1, -1 };
    uint32_t offsets[DMABUF_MAX_PLANES] = { 0 };
    uint32_t strides[DMABUF_MAX_PLANES] = { 0 };
    uint32_t fourcc = 0;
    uint64_t modifier = 0;
    int num_planes = 0;
    ...
    if (!force && gdpy->last_publish_dmabuf == dmabuf &&
        now_ms - gdpy->last_publish_ms < 16) {
        return;
    }
    ...
    if (!texture) {
        gdpy->publish_snapshot_fail_count++;
        if (gdpy->verbose || gdpy->publish_snapshot_fail_count <= 8) {
            warn_report("gvt-stream-publish: snapshot import failed");
        }
        return;
    }
    ...
    gdpy->publish_fb_index = (gdpy->publish_fb_index + 1) %
                             G_N_ELEMENTS(gdpy->publish_fb);
    pub_fb = &gdpy->publish_fb[gdpy->publish_fb_index];
    if (pub_fb->width != width || pub_fb->height != height) {
        egl_fb_destroy(pub_fb);
        egl_fb_setup_new_tex(pub_fb, width, height);
    }

    egl_fb_blit(pub_fb, &gdpy->guest_fb, qemu_dmabuf_get_y0_top(dmabuf));
    glFinish();

    if (!egl_dmabuf_export_texture(pub_fb->texture, fd,
                                   (EGLint *)offsets, (EGLint *)strides,
                                   (EGLint *)&fourcc, &num_planes,
                                   &modifier)) {
        gdpy->publish_snapshot_fail_count++;
        if (gdpy->verbose || gdpy->publish_snapshot_fail_count <= 8) {
            warn_report("gvt-stream-publish: snapshot export failed");
        }
        return;
    }

    snapshot = qemu_dmabuf_new(width, height, offsets, strides, 0, 0,
                               width, height, fourcc, modifier, fd,
                               num_planes, false, false);
    qemu_dmabuf_set_texture(snapshot, pub_fb->texture);
    gvt_stream_publish_dmabuf(gdpy, snapshot, now_ms, true);
    qemu_dmabuf_close(snapshot);
    gdpy->publish_snapshot_count++;
    gdpy->last_publish_dmabuf = dmabuf;
    gdpy->last_publish_ms = now_ms;

    if (gdpy->verbose || gdpy->publish_snapshot_count <= 8 ||
        gdpy->publish_snapshot_count % 60 == 0) {
        error_report("gvt-stream-publish: snapshot #%" PRIu64
                     " source=%s size=%ux%u",
                     gdpy->publish_snapshot_count,
                     gdpy->source_id ?: "vm", width, height);
    }
#else
    gvt_stream_publish_dmabuf(gdpy, dmabuf, now_ms, force);
#endif
}
                                     ...
                                     int width, int height)
{
    ...
    bool encode_h265 = gdpy->video_codec &&
    ...
    const char *raw_caps = encode_h265 ? "video/x-h265" : "video/x-h264";
...
static void gvt_stream_scanout_disable(DisplayChangeListener *dcl)
{
    ...
    egl_fb_destroy(&gdpy->guest_fb);
    egl_fb_destroy(&gdpy->capture_fb);
    for (int i = 0; i < G_N_ELEMENTS(gdpy->publish_fb); i++) {
                                 ...
                                 uint32_t w, uint32_t h)
{
    ...
    gvt_stream_publish_snapshot_dmabuf(gdpy, gdpy->scanout, now_ms, false);
