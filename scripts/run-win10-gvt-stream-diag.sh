...
RTP_PORT=${GVT_STREAM_RTP_PORT:-${GVT_STREAM_CONTROL_PORT:-5004}}
TAP_IF=tap-win10
BR_IF=br0
VM_DISK=${GVT_STREAM_VM_DISK:-/root/qemu_cmd/multivm/win10-vm1-active.qcow2}
VGPU_UUID=${GVT_STREAM_VGPU_UUID:-e582c6f5-e5cd-4a8e-9443-55a013e9f193}
...
if [ -n "$RTP_HOST" ] && [ -n "$RTP_PORT" ]; then
  ...
  DISPLAY_OPTS="${DISPLAY_OPTS},port=${RTP_PORT}"
fi
